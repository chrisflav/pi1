...
lemma diag_rename_comm_apply (p : MvPolynomial A R) :
    f ((MvPolynomial.eval Subtype.val) p) =
      (MvPolynomial.eval Subtype.val)
        ((MvPolynomial.map f) (MvPolynomial.rename hf.restrict p)) := by
...
lemma Ideal.mem_span_pow' {n : ℕ} (S : Set R) (x : R) :
  ...
  · rintro ⟨p, hp, rfl⟩
    ...
    rw [← @hp c (by simpa using hc)]
    have : Finsupp.degree = Finsupp.weight 1 :=
      Finsupp.degree_eq_weight_one (σ := S) (R := ℕ)
    rw [← this]
    rw [ Finsupp.degree, ← Finset.prod_pow_eq_pow_sum, Finsupp.prod]
...
variable
  (hcoeffsP : MvPolynomial.Set.coefficients (MvPolynomial.Set.coefficients (Set.range P)) ⊆ R₀)

noncomputable def hAux₀ (i : ι) : MvPolynomial ι R₀ := by
  apply RingOfDefinition.MvPolynomial.choosePreimageOfCoeffs (hAux f σ i)
  apply Set.Subset.trans
  · exact MvPolynomial.Set.coefficients_in
      (Set.range <| hAux f σ) (hAux f σ i) (Set.mem_range_self i)
  · intro r hr
    refine ⟨⟨r, ?_⟩, rfl⟩
    apply hcoeffsH
    exact hr
...
lemma hAuxSubXRep_eval (hspan : Ideal.span S = I) (i : ι)
    (hsig : AlgHom.comp (AlgHom.kerSquareLift f) σ = AlgHom.id R A) :
    (MvPolynomial.eval Subtype.val) (hAuxSubXRep f S σ hspan i hsig) =
      hAux f σ i - MvPolynomial.X i :=
...
lemma hAux₀_sub_X_in_I (i : ι) (hsig : AlgHom.comp (AlgHom.kerSquareLift f) σ = AlgHom.id R A) :
    MvPolynomial.map (SubringClass.subtype R₀)
      (hAux₀ f σ R₀ hcoeffsH i - MvPolynomial.X i) ∈ I := by
  simp only [map_sub, hAux₀_map, MvPolynomial.map_X]
  apply hAux_sub_X_in_I
  exact hsig

include S₀ hpreim₀ hcoeffsS hspan₀ in
@[simp]
lemma hAux₀_mod (hspan : Ideal.span S = I)
    (i : ι) (hsig : AlgHom.comp (AlgHom.kerSquareLift f) σ = AlgHom.id R A)
    (hcoeffsQ : MvPolynomial.Set.coefficients (MvPolynomial.coefficients
      (hAuxSubXRep f S σ hspan i hsig)) ⊆ R₀) :
  ...
  have hcQ :
      MvPolynomial.Set.coefficients (MvPolynomial.coefficients Q) ⊆
        Set.range (SubringClass.subtype R₀) := by
...
include S hpreim₀ P hPhom hcoeffsP hPeval hcoeffsS in
lemma exists_PAux₀ (s : MvPolynomial ι R₀) (hs : s ∈ S₀) :
  ...
  have hc : MvPolynomial.coefficients Ps ⊆
      Set.range (MvPolynomial.map (SubringClass.subtype R₀)) := by
    intro r hr
...
include hspan₀ hpreim₀ hcoeffsP S₀ hPeval hPhom hcoeffsH hcoeffsS in
lemma hAux₀_eval (a : MvPolynomial ι R₀) (ha : a ∈ I₀):
  ...
  | mem x h =>
    ...
    exact exists_PAux₀ f S σ R₀ hcoeffsS hcoeffsH P hPhom hPeval hcoeffsP S₀ hpreim₀ x h
  | zero => simp
  | add x y _ _ hx hy =>
    simp only [map_add]
...
theorem descent : ∃ (R₀ : Subring R) (A₀ : Type u) (_ : CommRing A₀) (_ : Algebra R₀ A₀)
  ...
  let coeffs_q : Set R := Set.iUnion
    (fun (i : ι) ↦ MvPolynomial.Set.coefficients
      (MvPolynomial.coefficients (hAuxSubXRep f S σ hS i hsig)))
  ...
  have hcoeffsQ (i : ι) :
      MvPolynomial.Set.coefficients
        (MvPolynomial.coefficients (hAuxSubXRep f S σ hS i hsig)) ⊆ R₀ := by
    apply Set.Subset.trans ?_ Algebra.subset_adjoin
    apply Set.Subset.trans ?_ Set.subset_union_right
    apply Set.subset_iUnion _ i
  have hcoeffsP : MvPolynomial.Set.coefficients
      (MvPolynomial.Set.coefficients (Set.range p)) ⊆ R₀ := by
    apply Set.Subset.trans ?_ Algebra.subset_adjoin
    apply Set.Subset.trans ?_ Set.subset_union_left
    exact Set.subset_union_right
  have hcoeffsH : MvPolynomial.Set.coefficients (Set.range (hAux f σ)) ⊆ R₀ := by
    apply Set.Subset.trans
    exact Set.subset_union_right (s := coeffs_s)
    apply Set.Subset.trans
    exact Set.subset_union_left (t := coeffs_p)
    apply Set.Subset.trans
    exact Set.subset_union_left (t := coeffs_q)
  ...
  have hS₀fin : Set.Finite S₀ := by
    ...
    · apply Set.injOn_of_injective
      apply MvPolynomial.map_injective
      simp only [SubringClass.coe_subtype]
      exact Subtype.val_injective
