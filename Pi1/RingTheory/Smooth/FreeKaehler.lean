...
structure Aux where
  ...
  s : S → P.Ring
  hs : ∀ (b : S), (algebraMap P.Ring S) (s b) = b
  g : P.Ring
  hgmem : g - 1 ∈ P.ker
...
def hom2 : P.toExtension.Cotangent →ₗ[S] S ⊗[D.T] D.Q₁.toExtension.Cotangent :=
  b.constr S fun i : σ ↦ 1 ⊗ₜ Extension.Cotangent.mk ⟨(D.f (b i)).val, D.vmem i⟩

def fhom : D.Q₁.Hom P where
  val i := X i
  aeval_val i := by
    simp only [aeval_X]
...
def hom1 : S ⊗[D.T] D.Q₁.toExtension.Cotangent →ₗ[S] P.toExtension.Cotangent :=
  LinearMap.liftBaseChange _ (Extension.Cotangent.map D.fhom.toExtensionHom)

lemma hspan : Submodule.span (M := D.Q₁.toExtension.Cotangent) D.T
...
def equiv1 : S ⊗[D.T] D.Q₁.toExtension.Cotangent ≃ₗ[S] P.toExtension.Cotangent := by
  refine LinearEquiv.ofLinear D.hom1 D.hom2 ?_ ?_
  · dsimp [hom1, hom2, fhom]
    ...
    exact D.hf (b i)
  · ext x
    have : x ∈ Submodule.span (M := D.Q₁.toExtension.Cotangent) D.T
        (Set.range <| fun i : σ ↦ Extension.Cotangent.mk ⟨(D.f (b i)).val, D.vmem i⟩) := by
      rw [hspan]
      trivial
...
lemma heq [Nontrivial S] :
    ((Generators.localizationAway D.gbar).ofComp D.Q₁.toGenerators).toAlgHom
        (D.P'.relation (Sum.inl ())) = C D.gbar * X () - 1 := by
  classical
  simp only [P', Q₂]
...
def equiv2 [Nontrivial S] : D.P'.toExtension.Cotangent ≃ₗ[S]
    (D.Q₂.toExtension.Cotangent × S ⊗[D.T] D.Q₁.toExtension.Cotangent) :=
...
lemma b_eq [Nontrivial S] (r : D.P'.rels) :
  ...
  · have : (D.bQ₂.prod D.bQ₁) (Sum.inl r) = (D.bQ₂ r, 0) := by simp
    ...
    have : (SubmersivePresentation.localizationAway S D.gbar).cotangentEquiv.symm
        ((SubmersivePresentation.localizationAway S D.gbar).basisDeriv r) =
          Extension.Cotangent.mk ⟨C D.gbar * X () - 1,
            (Presentation.localizationAway D.gbar (S := S)).relation_mem_ker ()⟩ := by
      apply (SubmersivePresentation.localizationAway S D.gbar).cotangentEquiv.injective
      simp
      ext j
      simp
      erw [PreSubmersivePresentation.cotangentComplexAux_apply]
      simp [SubmersivePresentation.localizationAway, PreSubmersivePresentation.localizationAway,
        Presentation.localizationAway]
    erw [this, Generators.cotangentCompLocalizationAwayEquiv_symm_inr]
    rfl
