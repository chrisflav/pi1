...
lemma diag_rename_comm_apply (p : MvPolynomial A R) :
    f ((MvPolynomial.eval Subtype.val) p) =
      (MvPolynomial.eval Subtype.val) ((MvPolynomial.map f)
        (MvPolynomial.rename hf.restrict p)) := by
...
noncomputable def setMapRepr (hpreim : MvPolynomial.map f ⁻¹' B = A)
...
lemma setMapRepr_homog (hpreim : MvPolynomial.map f ⁻¹' B = A) {n : ℕ}
    (hfinj : Function.Injective f) (homog : MvPolynomial.IsHomogeneous p n) :
