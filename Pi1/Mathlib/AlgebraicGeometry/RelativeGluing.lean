...
def PreRelativeGluingData.ρw {i j : d.ι} (hij : d.U i ≤ d.U j) :
    (Over.map (S.homOfLE hij)).obj (Over.mk (d.f i)) ⟶ Over.mk (d.f j) :=
  Over.homMk (d.ρ hij)

variable {S} (d : PreRelativeGluingData S) in
@[simp]
lemma PreRelativeGluingData.ρw_left {i j : d.ι} (hij : d.U i ≤ d.U j) :
    (d.ρw hij).left = d.ρ hij :=
  rfl
...
variable {S} (d : PreRelativeGluingData S) in
lemma PreRelativeGluingData.exists_mem (x : S) : ∃ i, x ∈ d.U i := by
  have : x ∈ (⊤ : S.Opens) := trivial
  obtain ⟨Us, hUs, hU⟩ := TopologicalSpace.Opens.isBasis_iff_cover.mp d.hU ⊤
  rw [hU] at this
  simp at this
  obtain ⟨a, ha, hx⟩ := this
  obtain ⟨i, rfl⟩ := hUs ha
...
instance {i j : d.ι} (hij : d.U i ≤ d.U j) : IsOpenImmersion (d.ρ hij) := by
  rw [← d.ρres_ι]
  infer_instance
...
private def 𝒱 (i : d.ι) : (d.aux hx ⁻¹ᵁ (d.U i)).toScheme.OpenCover where
  ...
  map j := IsOpenImmersion.lift (d.aux hx ⁻¹ᵁ (d.U i)).ι
    ...
    erw [Scheme.Cover.ι_glueMorphisms]
    exact y.property
  f x := 𝒰.f x.val
  covers z := by
...
def auxhOfLE {j : d.ι} (hji : d.U j ≤ d.U i) : (d.auxg f ⁻¹ᵁ d.U j).toScheme ⟶ d.X j :=
...
@[simps! -isSimp map]
def 𝒲new (j : d.ι) : (d.auxg f ⁻¹ᵁ d.U j).toScheme.OpenCover :=
...
@[simps!]
def 𝒲'new (j : d.ι) (k l : (d.𝒲new f j).J) :
  ...
  Scheme.OpenCover.mkOfCovers
    ...
    (fun x ↦ by
      ...
      have hinj : Function.Injective (pullback.fst ((d.𝒲new f j).map k)
          ((d.𝒲new f j).map l)).base := (Scheme.Hom.isOpenEmbedding _).injective
      apply hinj
      erw [← Scheme.comp_base_apply]
...
def auxhGeneralnew (j : d.ι) : (d.auxg f ⁻¹ᵁ d.U j).toScheme ⟶ d.X j :=
  let 𝒲 := d.𝒲new f j
...
lemma 𝒲new_map_auxhGeneralnew {j : d.ι} (k : (d.𝒲new f j).J) :
  ...
  simp only [auxhGeneralnew, Scheme.Cover.ι_glueMorphisms]
...
instance sheaf_representable : d.presheaf.IsRepresentable := by
  haveI inst : Presheaf.IsLocallySurjective Scheme.zariskiTopology (Sigma.desc d.subsheaf) :=
    inferInstance
  exact @Scheme.LocalRepresentability.isRepresentable
...
variable {d} in
def Glued.isoPullback (G : d.Glued) (i : d.ι) :
    (Over.pullback (d.U i).ι).obj (Over.mk G.f) ≅ Over.mk (d.f i) :=
  Over.isoMk (pullbackRestrictIsoRestrict G.f (d.U i) ≪≫ G.iso i)
...
/-- A choice of a glued scheme with isomorphisms over the `Uᵢ`. -/
def glued : d.Glued :=
  d.mkGluedFromRepresentable d.presheaf.reprX d.presheaf.representableBy

/-- The glued scheme represents `d.presheaf`. -/
def gluedRepresentable : d.presheaf.RepresentableBy d.glued.X :=
  d.presheaf.representableBy

/-- Morphisms into the glued scheme are given by compatible families over the relative
gluing datum `d`. -/
def gluedHomEquiv (T : Scheme.{u}) : (T ⟶ d.glued.X) ≃ d.CompatibleFamily T :=
  d.gluedRepresentable.homEquiv

lemma gluedHomEquiv_eq (T : Scheme.{u}) (f : T ⟶ d.glued.X) :
    d.gluedHomEquiv T f = d.presheaf.map f.op (d.gluedHomEquiv _ (𝟙 _)) :=
  Functor.RepresentableBy.homEquiv_eq _ _
...
variable {d} in
def Glued.ι (G : d.Glued) (i : d.ι) : d.X i ⟶ G.X :=
  (G.iso i).inv ≫ (G.f ⁻¹ᵁ d.U i).ι

variable {d} in
@[reassoc]
lemma Glued.f_ι (G : d.Glued) (i : d.ι) : d.f i ≫ (d.U i).ι = G.ι i ≫ G.f := by
  simp only [ι, Category.assoc, Glued.ι_f, Iso.inv_hom_id_assoc]

instance (G : d.Glued) (i : d.ι) : IsOpenImmersion (G.ι i) := by
  simp [Glued.ι]
  infer_instance

variable {d} in
@[reassoc (attr := simp)]
lemma Glued.iso_inv_ι (G : d.Glued) (i : d.ι) :
    (G.iso i).inv ≫ (G.f ⁻¹ᵁ d.U i).ι = G.ι i :=
  rfl

variable {d} in
@[reassoc (attr := simp)]
lemma Glued.isoPullback_inv_left_fst (G : d.Glued) (i : d.ι) :
    (G.isoPullback i).inv.left ≫ pullback.fst G.f (d.U i).ι = G.ι i := by
  simp [isoPullback]

@[simp]
lemma gluedRepresentable_homEquiv {T : Scheme.{u}} (f : T ⟶ d.glued.X) :
    d.gluedRepresentable.homEquiv f = d.gluedHomEquiv T f :=
  rfl

@[simp]
lemma gluedHomEquiv_id_g :
    ((d.gluedHomEquiv d.glued.X) (𝟙 d.glued.X)).g = d.glued.f :=
  rfl
...
variable {d} in
@[simps! map]
def Glued.openCover (G : d.Glued) : G.X.OpenCover :=
  Scheme.OpenCover.mkOfCovers d.ι d.X (fun i ↦ G.ι i) <| by
    intro x
    obtain ⟨i, hi⟩ := d.exists_mem (G.f.base x)
    use i, (G.iso i).hom.base ⟨x, hi⟩
    simp only [Scheme.comp_coeBase, TopCat.coe_comp, Function.comp_apply,
      Scheme.iso_hom_base_inv_base_apply, Glued.ι]
    rfl

variable {d} in
@[reassoc (attr := simp)]
lemma Glued.openCover_ρ_map (G : d.Glued) {i j : d.ι} (hij : d.U i ≤ d.U j) :
    d.ρ hij ≫ G.ι j = G.ι i := by
  simp only [ι]
  rw [← cancel_epi (G.iso i).hom, ← homOfLE_iso_hom_assoc]
  simp only [Iso.hom_inv_id_assoc, Scheme.homOfLE_ι]

variable {d}

@[simps! map]
def Glued.openCoverIntersection (G : d.Glued) (i j : d.ι) :
    (pullback (G.openCover.map i) (G.openCover.map j)).OpenCover :=
  Scheme.OpenCover.mkOfCovers { a | d.U a ≤ d.U i ∧ d.U a ≤ d.U j } (fun a ↦ d.X a)
    (fun a ↦ pullback.lift (d.ρ a.2.1) (d.ρ a.2.2) (by simp))
    (fun x ↦ by
      let xi : d.X i := (pullback.fst (G.openCover.map i) (G.openCover.map j)).base x
      let xj : d.X j := (pullback.snd (G.openCover.map i) (G.openCover.map j)).base x
      let y : G.X := (G.ι i).base xi
      let z : S := G.f.base y
      have hzi : z ∈ d.U i := by
        simp only [ι, Scheme.comp_coeBase, TopCat.coe_comp, Function.comp_apply, z, y]
        exact ((G.iso i).inv.base xi).property
      have hzj : z ∈ d.U j := by
        simp only [openCover_map, z, y, xi]
        rw [← Scheme.comp_base_apply]
        erw [← Scheme.comp_base_apply]
        rw [pullback.condition_assoc]
        exact ((G.iso j).inv.base xj).property
      obtain ⟨-, ⟨a, rfl⟩, hz, hle⟩ :=
        Opens.isBasis_iff_nbhd.mp (U := d.U i ⊓ d.U j) d.hU ⟨hzi, hzj⟩
      use ⟨a, le_trans hle inf_le_left, le_trans hle inf_le_right⟩
      use (G.iso a).hom.base ⟨y, hz⟩
      have : Function.Injective (pullback.fst (G.openCover.map i) (G.openCover.map j)).base :=
        (Scheme.Hom.isOpenEmbedding _).injective
      apply this
      have : Function.Injective (G.iso i).inv.base :=
        ConcreteCategory.injective_of_mono_of_preservesPullback _
      apply this
      simp only [openCover_map, Set.coe_setOf, Set.mem_setOf_eq]
      rw [← Scheme.comp_base_apply, pullback.lift_fst]
      rw [← Scheme.comp_base_apply]
      rw [← Glued.homOfLE_iso_hom]
      simp only [Scheme.comp_coeBase, TopCat.coe_comp, Function.comp_apply]
      rw [← Scheme.comp_base_apply]
      erw [← Scheme.comp_base_apply]
      rw [Category.assoc, Iso.hom_inv_id, Category.comp_id]
      simp only [openCover_map, y, xi]
    ...
    (fun a ↦ by
      apply @IsOpenImmersion.of_comp _ _ _ _ (pullback.fst _ _) ?_ ?_
      · apply IsOpenImmersion.pullback_fst_of_right
      · simp only [Set.coe_setOf, Set.mem_setOf_eq, openCover_map, limit.lift_π,
          PullbackCone.mk_pt, PullbackCone.mk_π_app]
        infer_instance)

def Glued.glueHoms (G : d.Glued) {T : Scheme.{u}}
    (g : ∀ i : d.ι, d.X i ⟶ T)
    (comp : ∀ {i j : d.ι} (hij : d.U i ≤ d.U j), d.ρ hij ≫ g j = g i) :
    G.X ⟶ T :=
  G.openCover.glueMorphisms g <| by
    intro i j
    apply (G.openCoverIntersection i j).hom_ext
    intro a
    simp [comp]

@[reassoc (attr := simp)]
lemma glueHoms_ι (G : d.Glued) {T : Scheme.{u}} (g : ∀ i : d.ι, d.X i ⟶ T)
    (comp : ∀ {i j : d.ι} (hij : d.U i ≤ d.U j), d.ρ hij ≫ g j = g i) (i : d.ι) :
    G.ι i ≫ G.glueHoms g comp = g i := by
  rw [Glued.glueHoms, ← G.openCover_map, Scheme.Cover.ι_glueMorphisms]

lemma Glued.hom_ext {G : d.Glued} {T : Scheme.{u}} (f g : G.X ⟶ T)
    (h : ∀ i : d.ι, G.ι i ≫ f = G.ι i ≫ g) : f = g := by
  apply G.openCover.hom_ext
  intro i
  simp [h i]

def Glued.glueHomsOver (G : d.Glued) {T : Over S}
    (g : ∀ i : d.ι, d.X i ⟶ T.left)
    (comp : ∀ {i j : d.ι} (hij : d.U i ≤ d.U j), d.ρ hij ≫ g j = g i)
    (w : ∀ i : d.ι, g i ≫ T.hom = d.f i ≫ (d.U i).ι) :
    Over.mk G.f ⟶ T :=
  Over.homMk (G.glueHoms g comp) <| by
    apply Glued.hom_ext
    intro i
    simp only [Functor.const_obj_obj, Over.mk_left, glueHoms_ι_assoc, Over.mk_hom]
    rw [w, Glued.f_ι]

@[simp]
lemma Glued.ι_glueHomsOver (G : d.Glued) {T : Over S}
    (g : ∀ i : d.ι, d.X i ⟶ T.left)
    (comp : ∀ {i j : d.ι} (hij : d.U i ≤ d.U j), d.ρ hij ≫ g j = g i)
    (w : ∀ i : d.ι, g i ≫ T.hom = d.f i ≫ (d.U i).ι) :
    (G.glueHomsOver g comp w).left = G.glueHoms g comp :=
  rfl

def openCover : S.OpenCover := Scheme.OpenCover.mkOfCovers d.ι (fun i ↦ d.U i) (fun i ↦ (d.U i).ι)
  (fun x ↦ by
    obtain ⟨i, hx⟩ := d.exists_mem x
    use i, ⟨x, hx⟩
    rfl)

lemma Glued.f_prop {P : MorphismProperty Scheme.{u}} [IsLocalAtTarget P]
    (G : d.Glued) (h : ∀ i, P (d.f i)) : P G.f := by
  apply IsLocalAtTarget.of_openCover d.openCover
  intro i
  show P (pullback.snd G.f (d.U i).ι)
  have : (G.isoPullback i).hom.left ≫ d.f i = pullback.snd G.f (d.U i).ι := (G.isoPullback i).hom.w
